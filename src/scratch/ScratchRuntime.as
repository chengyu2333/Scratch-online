		...
		public function saveRecording():void {
			...
			function saveAndUploadFile():void {
				...
				if(new Server().URLs['OSS']&&new Server().URLs['OSS']!=""){
					var posturl:String = new Server().URLs['OSS']+"/video/"+app.user_id+"/"+app.getTime()+" | "+ app.projectName() +".flv?append&position=0";
					var url:String = new Server().URLs['OSS']+"/video/"+app.user_id+"/"+app.getTime()+" | "+ app.projectName() +".flv";
				}else{
					var posturl:String = new Server().URLs['siteAPI']+"?r=api/upload&user_id="+app.user_id+"&user_token="+app.user_token+"&user_class_id="+app.user_class_id+"&type=0&filename="+app.projectName()+".flv";
					var url:String = "";
				}
			...
			function saveFile():void {
				...
				DialogBox.close("播放分享你的视频",null,specEditor,"关闭");
				releaseVideo(false);
			}
